...
## File Structure
- **Magic header**: `6d 61 76 2a` (`mav*`)
- **Bytes 4–7**: Unknown (possibly size/version)
- **Bytes 8+**: Raw DEFLATE compressed data (`pako.inflateRaw(data.slice(8))`)
- **Decompressed**: JSON wrapper `{"Data": "<inner JSON string>", "Digest": "<json string>"}`
- **Inner JSON quirks**:
  - `"otherstats":{{` → fix to `"otherstats":{` (double-brace bug)
  - Missing commas between `}{` sequences → insert `,`
- **Digest**: A separate JSON object with per-character metadata (`Name`, `Level`); used as fallback display values
...
Three plain `<script>` tags in `index.html`, no server required (works from `file://`):

1. **`maxguids.js`** — Blueprint GUID dictionaries (`DH_GUIDS`, `QUALITY_GUIDS`, `LEGENDARY_AFFIX_NAMES`). Auto-extracted game data; do not hand-edit.
2. **`maxparser.js`** — All parsing logic, curated constants (`STAT_GUIDS`, `CLASS_CONFIG`, `_HEART_NAMES_MAP`, etc.), and public functions (`parseMaxFile`, `processCharacterData`).
3. **`app.js`** — UI rendering and interaction.
...
| `Melee Swamp Husk Unique Unit` | Char Root Heart |
